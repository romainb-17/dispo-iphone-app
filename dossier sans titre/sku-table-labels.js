...
async function loadMap(){
  try{
    const src = (typeof DATA_URL !== "undefined" && DATA_URL) || "/skus-fr.json";
    const url = new URL(src, location.origin);
    url.searchParams.set("v", String(Date.now())); // cache-bust
    const r = await fetch(url.toString(), { cache: "no-store" });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    const text = await r.text();
    try { return buildMap(JSON.parse(text)); }
    catch(e){ console.warn("[SKU_LABELS] JSON parse KO:", e); return {}; }
  }catch(e){
    console.warn("[SKU_LABELS] fetch KO:", e);
    return {};
  }
}
...
;(() => {
  ...
  // Helper robuste : choisit ensureMap global si dispo, sinon tombe à 0
  window.__skuMapSize = async function () {
    try {
      const fn = (typeof ensureMap === "function")
        ? ensureMap
        : (typeof window.ensureMap === "function" ? window.ensureMap : null);
      if (!fn) return 0;
      const m = await fn();
      return Object.keys(m || {}).length;
    } catch (e) {
      console.warn("[debug] __skuMapSize error", e);
      return -1;
    }
  };

  window.__skuRelabel = function () {
    try {
      if (typeof run === "function") return run();
      if (typeof window.run === "function") return window.run();
    } catch (e) {
      console.warn("[debug] __skuRelabel error", e);
    }
    return null;
  };
})();
